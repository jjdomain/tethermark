...
For an unattended trusted-team instance, do not run the application from a maintainer account or writable checkout. Follow [Least-Privilege Shared-Service Deployment](./shared-service-deployment.md) for dedicated systemd, launchd, and Windows Task Scheduler identities, external protected data/configuration paths, filesystem permissions, and the API-provider requirement for service workloads.
