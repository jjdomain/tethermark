...
- Every completed audit now emits a `2026-08-26.run-versions.v2` manifest for methodology, static baseline, control catalog, policy, audit-package catalog, prompt set, resolved learning-overlay version, tool versions/capabilities, and model identities. Static findings now carry a file/line or artifact citation.
...
- The project owner accepted deferral of executable Docker Desktop validation on a real Mac on 2026-08-24. Phase 8 is closed for the current CE scope without certifying macOS runtime execution; macOS retains native build/fail-closed boundary evidence and static support.
...
- Runtime readiness and real audit-target execution now pass on Windows Docker Desktop plus native Linux Docker, rootless Podman, and gVisor with persisted plan/result/cleanup evidence and no host fallback. Real macOS Docker Desktop execution is explicitly deferred and excluded from certified runtime claims.
...
Status: **Complete — real-Mac execution explicitly deferred and excluded from certified claims**
...
- [x] Record the project-owner decision to defer executable `docker_desktop` validation on a real Mac; do not treat the hosted macOS blocked-boundary job as runtime-execution evidence.
...
- The same run passed the macOS 15 arm64 blocked-boundary gate. It proves native build and fail-closed backend resolution, not Docker Desktop execution; the project owner accepted deferral of the executable real-Mac gate for the current CE scope.
...
- [x] Finish retention scheduling and ensure database rows/artifacts remain consistent after pruning. The API now runs bounded 30-day run-artifact and 7-day sandbox maintenance by default, protects active durable jobs, preserves normalized SQLite history, reconciles stale raw-artifact index rows, retries failed cycles, and retains a bounded local maintenance history.
- [x] Decide how promoted learning overlays are consumed by planner/prompts/rules, with strict versioning and rollback. Future runs resolve only active, unexpired, explicitly human-approved overlays; additive evidence/validation and bounded advisory context are consumed, permissive suppression/severity overlays remain non-executable, each run persists a content-versioned resolution, reuse invalidates on version change, and rollback removes the effect from subsequent runs.
...
- [x] Package or securely bootstrap required browser and static/runtime tools with pinned checksums. `scripts/toolchain-lock.json` and `npm run toolchain:check` bind exact SHA-512 npm integrity for Playwright/Playwright Core, the SHA-256 of Playwright's embedded browser-revision manifest, exact browser revisions/versions, every direct static-scanner archive/wheel SHA-256 allowlist, and all runtime workload image SHA-256 digests. `npm run setup:browser` uses only the installed Playwright CLI, supports an explicit preview/confirmation flow, and launches each installed browser to verify its version; the release browser matrix uses this path instead of `npx`. Runtime engines remain operator-confirmed externally signed OS/vendor packages rather than Tethermark-downloaded archives; capability verification and recorded versions are bounded by the published release support matrix.
...
- [x] Run dependency, license, secret, Semgrep, Trivy, and Scorecard release checks with triage policy. `npm run release:security` now executes a fail-closed, checksum-bound, secret-safe gate over the releasable checkout with checked-in severity/license/Scorecard floors and exact, approved, maximum-90-day exceptions. The authenticated 2026-08-28 Windows run completed every scanner: npm advisories, dependency licenses, Semgrep, and all Trivy categories passed after ignored private/runtime workspaces were removed from release scope; release remains explicitly blocked by the missing root license declaration/file and the remote Scorecard 3.9 overall score against the unchanged 5.0 floor.
- [ ] Produce an SBOM and signed/checksummed release artifacts; document verification. Implementation is ready: deterministic tag-bound `.zip`/`.tar.gz` source archives, a canonical CycloneDX 1.5 SBOM covering the npm graph plus 92 hash-locked Python requirements, `release-manifest.json`, exact `SHA256SUMS`, cross-platform verification/tamper tests, and a tag/manual-only GitHub OIDC workflow using commit-pinned `actions/attest` for SLSA provenance and SBOM predicates. Local double-build and tamper validation pass, but this task remains open until the repository license and Scorecard blockers are resolved and an actual tagged workflow run produces signatures that pass the documented independent verification commands.
...
- [ ] Validate clean install and upgrade on Windows, macOS, Ubuntu, and the supported container/server deployment. Implementation is ready: `test:install-upgrade` now performs an actual isolated clone, lockfile install, build, detached revision marker check, two-ref update, protected configuration/state preservation check, clean-checkout assertion, and installed API/UI smoke launch. The manually dispatched/path-gated **Install And Upgrade Verification** workflow runs it on explicit Ubuntu 24.04, Windows Server 2025, and macOS 15 runners, retains sanitized JSON evidence, and adds the hardened server-profile contract on Ubuntu. Local Windows first-run execution exposed and fixed Node 24's rejection of direct `spawnSync("npm.cmd")` by invoking npm's JavaScript CLI through Node. This task remains open until the committed candidate workflow produces passing retained evidence on all three native runners; macOS container execution remains explicitly deferred.
