...
- [ ] Build versioned eval packs for all runtime controls defined in Phase 7 (prompt/secret/tool, indirect-injection/data-exfiltration/cross-session-memory, MCP, unsafe-output, and excessive-agency packs are complete; resource and telemetry packs remain).
...
1. Continue versioned runtime coverage for resource and telemetry controls using the now-executable adapter boundary.
