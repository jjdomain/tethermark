...
- [ ] Security review, dependency/license review, SBOM, artifact checksums/signing, docs, and rollback notes are complete. On 2026-08-29, the live candidate-bound security gate passed from clean commit `3088f0c` for proposed tag `v0.2.2`: npm dependency vulnerabilities and licenses, repository licensing, Semgrep, Trivy vulnerabilities/licenses/secrets/misconfigurations, and authenticated OpenSSF Scorecard all reported zero blockers with no applied exceptions. The deterministic artifact gate reproduced both archives and verified manifest, CycloneDX SBOM coverage (162 components, including 93 hash-locked Python components), SHA-256 checksums, and tamper rejection. A final unsigned build from the same commit produced four verified artifacts, and the release notes document installation, compatibility, platform/provider limits, and exact rollback to immutable `v0.2.1`. This item remains open only for the final tag-bound OIDC signing, SLSA provenance, and CycloneDX attestation; those official artifacts must not be generated before every release-candidate gate closes.
