...
Community Edition Phases 1–11 are complete within the documented platform scope. Runtime evidence now has bounded confidence labels, explicit within-run and independent-run sample limits, semantic repeatability fingerprints, and fail-closed worker handling. Persistence recovery, local retention, human-approved and reversible learning, synthesis authorization, learning-input minimization, and the CE/hosted ownership boundary are closed. The Phase 12 commit-bound `0.2.2` lifecycle/recovery gate passes fresh install, two-ref upgrade, backup/restore, cancellation/restart recovery, policy migration, and export compatibility.

1. Complete the remaining Phase 12 security/artifact/documentation and System Policies release-candidate gates.
2. Run the consented beta and production-launch evidence, then create and publish `v0.2.2` only after maintainer acceptance.
