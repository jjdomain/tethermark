...
- reviewed publication now distinguishes the immutable automated decision from the effective export decision: an explicit `approve_run` action can elevate only a public-recommended, public-safe `review_required` result whose active gating findings are all explicitly public-safe; internal, unsafe, blocked, and non-public-safe results remain non-publishable
- source adjudication of the Promptfoo calibration run added placeholder-secret, same-repository image-optimization permission, and fully escaped shell-argument corrections; its v3 result remains calibration-only because Scorecard timed out and every retained finding required manual review, and public cohort candidates must be reassessed under implementation v4
- production cohort scans use the fixed versioned four-provider evidence plan through the explicit CLI evidence-policy option, so model selection cannot silently omit Scorecard, Semgrep, or Trivy from a leaderboard candidate
- eligible public GitHub cohort scans now fall back from a locally unavailable, blocked, timed-out, or runtime-failed Scorecard invocation to the public Scorecard API; a completed fallback is recorded and satisfies export completeness without hiding the failed direct attempt
...
- the first withheld AISecurityBase SDK pilot produced source-backed analyzer corrections for `uv.lock`, redacted test-fixture credentials, inert Python examples, and sandbox API execution; production credential and host-shell detections remain fail-closed
...
Community Edition Phases 1–12 are complete within the documented platform scope. Runtime evidence has bounded confidence labels, explicit within-run and independent-run sample limits, semantic repeatability fingerprints, and fail-closed worker handling. Persistence recovery, local retention, human-approved and reversible learning, synthesis authorization, learning-input minimization, and the CE/hosted ownership boundary are closed. Immutable `v0.2.3` is published with the post-launch static-evidence, provider-selection, deduplication, score-labeling, and placeholder-secret corrections, backed by exact-tag deterministic, System Policy, live security, signed artifact, SLSA provenance, and CycloneDX evidence. The supported product boundary is unchanged, and immutable `v0.2.2` remains the rollback target.

1. Monitor `v0.2.3` for audit-quality regressions, authentication/rate failures, runtime cleanup, scoring drift, and upgrade failures; apply the documented rollback criteria when necessary.
2. Keep future methodology/catalog expansion and additional representative OSS samples versioned as post-launch work. Do not reopen completed release gates without a qualifying defect or contract change.
