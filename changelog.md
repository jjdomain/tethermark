...
- Approved public-safe reviews now resolve to an effective `publishable` executive-export status when every active gating finding is itself explicitly public-safe. Exports preserve the automated publishability and review-required decisions alongside the effective status and approval basis; approval cannot elevate internal, unsafe, blocked, or non-public-safe results.
- Executive validation completeness now accepts persisted successful model calls and distinguishes expected static scope gaps—runtime controls plus branch-protection and provenance settings—from unexpected missing assessment. Fully completed static evidence no longer receives a false incomplete warning merely because runtime-only controls remain out of scope.
- Scorecard runtime failures and timeouts now activate the public Scorecard API fallback for eligible GitHub repositories, the local timeout can be bounded independently with `HARNESS_SCORECARD_TIMEOUT_MS`, and executive completeness recognizes a completed fallback as satisfying the requested provider.
- Token-permission scoring now treats negative Scorecard checks as not applicable and recognizes explicitly purpose-bound `contents: write` grants for Pages, docs, publishing, deployment, release, and same-repository image-optimization workflows instead of claiming they are unnecessarily broad. `write-all` and unexplained write grants remain findings.
- Python `uv.lock` files now count as dependency lock evidence; documentation placeholders such as `your_openai_api_key`, `e2b_xxx_your_key_here`, and `sk_xxx` are excluded; generic credential-like literals confined to tests remain redacted observations unless they match a known credential format; and test strings, Python triple-quoted examples, sandbox API `shell=True` calls, and fully POSIX-escaped shell argument vectors no longer become host-shell agent findings. Production credentials and real unsafe `subprocess(..., shell=True)`, `os.system`, and Node `child_process.exec` paths remain findings.
- Purpose-bound sentinel literals wrapped in double underscores, such as `__model_info_lookup__`, are now treated as placeholders instead of production credentials; credential-shaped values and ordinary non-sentinel literals remain eligible for secret review.
- The noisy-fixtures validation contract now requires explicitly synthetic, test-only credentials to stay redacted review evidence without producing a production secret-exposure finding.
...
- Static cohort CLI runs can now request the versioned fixed four-provider evidence plan directly with `--evidence-plan-policy`, giving production leaderboard audits the same non-downgradable repo-analysis, Scorecard, Semgrep, and Trivy coverage as calibration runs.
...
- Phase 11 now has a canonical CE threat model for credentials, Codex session discovery, logs/artifacts, webhooks, archives, repository clones, and runtime mounts. Durable jobs reject inline model keys; settings responses mask signing secrets; local Unix secret/artifact storage is owner-only; webhook targets, archive entries, Git URLs/environments, and nested logs now have explicit fail-closed guards covered by the native release matrix.
- Phase 11 now keeps the API and web UI on loopback by default and rejects non-loopback startup unless API-key authentication, a 32-character minimum key, and an exact network-exposure acknowledgement are configured. Direct and combined launchers share the policy, permitted exposure warns that TLS termination is external, and the fail-closed regression runs across the native CI matrix.
- Phase 11 now has a checked-in acquisition integrity lock for Playwright packages/browser revisions, static scanner archives and wheels, and digest-addressed runtime images. Browser setup uses the locally installed, integrity-checked Playwright CLI with preview/confirmation and post-install launch/version verification; the release matrix no longer invokes `npx`.
- Phase 11 now provides ref-pinned Windows and Unix install/update/rollback paths, a lockfile-enforced `npm run first-run` setup and doctor, a local revision marker, and guarded uninstall scripts that verify their target and preserve checkout-local state by default. The native lifecycle regression is part of CI and `release:check`; documentation no longer advertises unpublished hosted installer URLs.
