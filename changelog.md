...
- Added executable clean-install and two-ref upgrade validation with state-preservation and post-update API/UI launch assertions, a retained Windows/Ubuntu/macOS workflow, and an Ubuntu server-profile gate. Fixed Windows Node 24 first-run execution to invoke npm through its JavaScript CLI instead of spawning `npm.cmd` directly, and made the API/UI smoke check terminate its Windows process tree without reporting the intentional cleanup as a failure.
