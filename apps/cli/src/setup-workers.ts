...
export function buildPythonWorkerSetupPlan(options: { workspaceRoot?: string; python?: string; venvRoot?: string } = {}): PythonWorkerSetupPlan {
  ...
  const reason = !basePython
    ...
    : !lockSha256
      ...
      : null;
  const steps: PythonWorkerSetupStep[] = basePython && lockSha256 ? [
    {
      label: "Stage the hash-locked environment bootstrap",
      command: basePython.command,
      args: [...basePython.prefix_args, "-m", "pip", "install", "--require-hashes", "--only-binary=:all:", "--no-deps", "--upgrade", "--target", bootstrapRoot, "-r", lockPath]
    },
    {
...
export function runSetupWorkers(options: { dryRun?: boolean; yes?: boolean; python?: string; venvRoot?: string } = {}): PythonWorkerEnvironmentInspection | null {
  ...
  for (const [name, version] of Object.entries(expectedPackages)) {
    if (installedPackages[name] !== version) throw new Error(`Locked Python package mismatch for ${name}: expected ${version}, found ${installedPackages[name] ?? "missing"}.`);
  }
