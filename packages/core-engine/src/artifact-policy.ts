...
const exactPolicies = new Map<string, ArtifactPolicy>([
  ...
  ["learning-overlay-resolution", { disposition: "queryable_persisted", persisted_table: "stage_artifacts", rationale: "The exact approved learning overlay set and version consumed by a run are persisted for replay, reuse invalidation, and rollback verification." }],
  ...
  ["finding-integrity-pre-supervisor", { disposition: "artifact_only", persisted_table: null, rationale: "Pre-supervisor evidence and integrity facts remain archival debug input for supervisor/correction review." }],
  ["post-supervisor-integrity", { disposition: "queryable_persisted", persisted_table: "finding_quality", rationale: "Post-supervisor deterministic integrity validation is persisted as normalized run-level gate output." }],
  ["finding-quality", { disposition: "queryable_persisted", persisted_table: "finding_quality", rationale: "Compatibility alias for post-supervisor deterministic integrity validation." }],
  ["finding-quality-pre-skeptic", { disposition: "artifact_only", persisted_table: null, rationale: "Deprecated compatibility alias for pre-supervisor evidence and integrity facts." }],
